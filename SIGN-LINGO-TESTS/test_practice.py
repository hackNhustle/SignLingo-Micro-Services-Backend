...
def test_practice_direct_bypass(api_client):
    """
    Attempts to hit the practice service directly bypassing Kong. 
...
def test_practice_kong_no_jwt(api_client):
...
def test_practice_kong_with_valid_jwt(api_client, auth_headers):
    """Hits Kong with a mathematically valid JWT. 
    Kong should accept it, dynamically inject X-Consumer headers, 
    and the Practice service should implicitly trust and process it."""
